#if !defined(TRADER_HANDLER_H)
#define TRADER_HANDLER_H
...
{
public:
	CTraderHandler(CThostFtdcTraderApi *pUserApi);
	~CTraderHandler() {}
	...
	//请求查询合约
	void ReqQryInstrument(std::map<std::string, std::vector<std::string>> *product_instrument_list);
...
private:
	...
	///认证码
	TThostFtdcAuthCodeType	m_chAuthCode;
	///App代码
	TThostFtdcAppIDType	m_chAppID;

	std::vector<std::string> m_sProductID;	//要订阅的产品id
	int m_nRequestID;

	std::map<std::string, std::vector<std::string>>	*m_product_Instrument_list;//产品对应的合约列表
};
